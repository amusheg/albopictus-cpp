...
minwintertemp_us<-filter(tmin_us, month %in% c("Nov","Dec","Jan","Feb")) %>% group_by(pop, seasonalyear) %>% summarise(wintermin=min(Tmin), wintermonths_sd=sd(Tmin)) %>% filter(!(seasonalyear %in% c(1997,2018)))
...
minwintertemp_jp<-filter(tmin_jp, month %in% c("Nov","Dec","Jan","Feb")) %>% group_by(pop, seasonalyear) %>% summarise(wintermin=min(Tmin), wintermonths_sd=sd(Tmin)) %>% filter(!(seasonalyear %in% c(1997,2018)))
...
ggplot(filter(gdd400_all_new, year==2007, CPP_YEAR==2018), aes(x=gdd400, y=Estimate, color=country)) + geom_point() + stat_smooth(method=lm) 
```
```{r}
