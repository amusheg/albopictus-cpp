...
colnames(cpp_2019)[colnames(cpp_2019)=="Std..Error"] <- "StdError"
...
#find closest day length in vector
...
cpp_2$date_again<-format(as.Date(cpp_2$date), "%mm-%dd")
cpp_2$date_again<-as.Date(cpp_2$date_again, format="%mm-%dd")
ggplot(filter(cpp_2, year!=1988), aes(x=lat, y=date_again, color=year)) + geom_point() + scale_y_date(date_breaks = "weeks" , date_minor_breaks="days", date_labels = "%b-%d") + facet_grid(~country) + theme_bw()
```
