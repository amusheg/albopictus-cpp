...
country_DI_bw<-ggplot(filter(DI_short_2, year!=1988), aes(x=lat, y=DI, fill=country, shape=year)) + scale_fill_manual(values=c("black","gray"), name="Country") + scale_shape_manual(values=c(21,24), name="Year") + geom_point(size=3) + theme_bw() + theme(legend.position=c(0.9,0.3), text=element_text(size=14)) + guides(fill = guide_legend(override.aes=list(shape=22))) + xlab("Latitude") + ylab("Short-day diapause incidence")
=======
...
>>>>>>> b9e6394c79cf3ffa4de3228b1d1aa580a816f24b
...
hours<-
-(cppmodel$coefficients["(Intercept)"])/cppmodel$coefficients["photoperiod"]
hours
