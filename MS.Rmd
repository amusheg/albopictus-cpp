...
ggplot(filter(cpp_2, year!=1988), aes(x=lat, y=Estimate, fill=country)) + 
geom_point(size=3, shape=21) + scale_fill_manual(values=c("black", "gray"), name="Country") + stat_smooth(method=lm, aes(color=country)) + scale_color_manual(values=c("black","gray"), name="Country") +
  facet_grid(~year) +
  guides(fill=guide_legend(override.aes=list(shape=21))) +
  theme_bw() + theme(legend.position=c(.8,.25), text=element_text(size=14)) + 
...
plot_grid(us_map_bw, japan_map_bw)
...
ggplot(filter(DI_short_2, year!=1988), aes(x=lat, y=DI, color=country, shape=year)) + scale_color_manual(values=c("red","blue")) + geom_point(size=3) + theme_bw() + theme(legend.position=c(0.9,0.3)) + stat_smooth(aes(linetype=year, color=country), se=FALSE) + scale_linetype_manual(values=c("dotted","solid")) + xlab("Latitude") + ylab("Short-day diapause incidence")

ggplot(filter(DI_short_2, year!=1988), aes(x=lat, y=DI, fill=country, shape=year)) + scale_fill_manual(values=c("black","gray")) + 
scale_shape_manual(values=c(21,24)) + geom_point(size=3) + theme_bw() + theme(legend.position=c(0.9,0.3)) + stat_smooth(aes(linetype=year, color=country), se=FALSE) + scale_color_manual(values=c("black","gray")) + scale_linetype_manual(values=c("dotted","solid")) + xlab("Latitude") + ylab("Short-day diapause incidence")
...
ggplot(blockAB, aes(x=TotalLarvaeEmbryos, y=yield)) + geom_point() + xlab("Number of viable eggs on oviposition paper") + ylab("Viable eggs/initial egg count on paper") + theme_bw()
```
...
```{r}
#Dose response curve plot from Jen
#cpp<-read.table("CPP.csv", header=TRUE, sep=",")
#cpp

#fm1<-cbind(hatched, nothatched)~photoperiod
#cppmodel<-glm(fm1, data=cpp, family=binomial())

#summary(cppmodel)

#fitteddata1<-predict(cppmodel,type="response")
#fitteddata1

#plot(cpp$photoperiod, fitteddata1, ylab="Probability of hatching",
 #    xlab="photoperiod", ylim=c(0,1))
  #   lines(cpp$photoperiod,fitteddata1, lty=1)
#y<-cpp$hatched/(cpp$hatched+cpp$nothatched)
#text(cpp$photoperiod,y)
#hours<-(cppmodel$coefficients["(Intercept)"])/cppmodel$coefficients["photoperiod"]
#hours
