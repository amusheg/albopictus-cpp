...
#version with only one shape, gray/black for years
ggplot(cpp_all, aes(x=lat, y=Estimate, alpha=year, fill=country)) +
  geom_point(size=3, shape=21, stroke=1) + scale_fill_manual(values=c("white", "black"), name="Country") +
  scale_alpha_manual(values=c(0.3, 1), name="Year") +
  stat_smooth(method=lm, se=FALSE, aes(linetype=country, color=year)) + scale_color_manual(values=c("gray", "black"), name="Year") +  scale_linetype_manual(values=c("dotted","solid"), name="Country") + 
  theme_bw() + theme(legend.position=c(.8,.25), text=element_text(size=12)) + xlab("Latitude") + ylab("Critical photoperiod (h)") + guides(linetype=guide_legend(override.aes=list(color="black"), legend.key.width=unit(5,"line")))
...
dimodel_poly_us<-lm(DI~lat^2 + lat, data=filter(DI_short_2, country=="US"))
dimodel_lat_us<-lm(DI~lat, data=filter(DI_short_2, country=="US"))
...
cor.test(filter(DI_short_2, country=="US", year==2018)$lat, filter(DI_short_2, country=="US", year==2018)$DI, alternative="two.sided", method="spearman")
cor.test(filter(DI_short_2, country=="JP", year==2018)$lat, filter(DI_short_2, country=="JP", year==2018)$DI, alternative="two.sided", method="spearman")



wilcox.test(filter(DI_short_2, country=="US", year==2018, pop!="PBS")$DI, (filter(DI_short_2, country=="US", year==2008) %>% arrange(lat))$DI, paired=FALSE)
...
```{r, echo=FALSE, include=FALSE}
gdd_deadline_plots<-ggplot(filter(gdd465_all, only_2008=="n", year!=2018), aes(x=latitude, y=gdd465, fill=country)) + scale_fill_manual(values=c("black", "gray"), name="Country") + geom_point(shape=21) + facet_wrap(~year) + xlab("Latitude") + ylab("GDD Deadline (ordinal day)") + theme_bw() + theme(legend.position="bottom")
...
gdd_08 <- ggplot(filter(gdd465_all_new, year==2007, CPP_YEAR==2008), aes(x=gdd465, y=Estimate, fill=country)) + scale_fill_manual(values=c("black","gray"), name="Country") + geom_point(size=3, shape=21) + stat_smooth(method=lm, se=FALSE, aes(color=country)) + scale_color_manual(values=c("black","gray"), name="Country") +
...
gdd_18 <- ggplot(filter(gdd465_all_new, year==2017, CPP_YEAR==2018), aes(x=gdd465, y=Estimate, fill=country)) + scale_fill_manual(values=c("black","gray"), name="Country") + geom_point(size=3, shape=21) + stat_smooth(method=lm, se=FALSE, aes(color=country)) + scale_color_manual(values=c("black","gray"), name="Country") +
...
#field eggs only
field.model.fit <- drm(DI ~ Week, data = dc_field_eggs, fct = LL.5())
summary(field.model.fit)
# estimate the 50% intercept
cpp_field<-data.frame(ED(field.model.fit, 50))


field_data_plot<-ggplot(field_data, aes(x=WeekDate, y=DI)) + geom_point(size=3, shape=21, fill="gray") + theme_bw() + theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
xlab("Week") + ylab("Diapause incidence") + geom_vline(aes(xintercept=cpp_field$Estimate), linetype="dotted")


#2014 GDD diagram
man2014<-read.csv("2014_Mean_Temp_MAN.csv") %>% gather(degreetype, degrees, "CountDegrees":"UncountDegrees")

man2014_gdd_plot<-ggplot(filter(man2014, DOY_2014>187), aes(x=DOY_2014, y=degrees, fill=degreetype)) + geom_bar(stat="identity", width=1) + 
  scale_fill_manual(values=c("dark gray","light gray")) + 
  theme_bw() + 
  geom_hline(yintercept=10, linetype="dotted") + geom_vline(xintercept=251) + theme(legend.position="none") +
  ylab("Mean Temp (C)")
...
```{r table1, echo=FALSE}
kable(filter(pops, only_2008=="n")[,1:7], col.names=c("Population", "City", "Latitude", "Longitude", "Country", "Generation*", "Life Stage Sampled"), caption="Table 1: 2018 Sampling Locations")
```
...
```
```{r echo=FALSE, include=FALSE}
plot_grid(ggplot(filter(gdd465_jp, only_2008=="n"), aes(x=year, y=gdd465, color=pop)) + geom_point() + geom_line() + theme_bw() + stat_smooth(method=lm, se=FALSE) + xlab("Year") + ylab("GDD Deadline"),ggplot(gdd465_us, aes(x=year, y=gdd465, color=pop)) + geom_point() + geom_line() + theme_bw() + stat_smooth(method=lm, se=FALSE) + xlab("Year") + ylab("GDD Deadline"), labels="AUTO")

```

```{r echo=FALSE, warning=FALSE}
gdd_by_interval<-ggplot(filter(interval_summaries, CPP_YEAR==2018), aes(x=latitude, y=gdd465_mean, fill=country, shape=interval)) + 
  geom_point(size=3) + scale_fill_manual(values=c("black", "gray"), name="Country") + scale_shape_manual(values=c(21,24), name="Period") + geom_errorbar(aes(ymin=gdd465_mean-gdd465_sd, ymax=gdd465_mean+gdd465_sd, color=country)) +
...
```
```{r}
annualmeantemp_US<-read.csv("AnnualMeanTemp_US.csv", check.names=FALSE) %>% gather(year, annualmeantemp, "1998":"2018", factor_key=FALSE)
annualmeantemp_US$year<-as.numeric(annualmeantemp_US$year)
#ggplot(annualmeantemp_US, aes(x=year, y=annualmeantemp, color=pop)) + geom_point() + geom_line()

annualmeantemp_JP<-read.csv("AnnualMeanTemp_JP.csv", check.names=FALSE) %>% gather(year, annualmeantemp, "1998":"2018", factor_key=FALSE)
annualmeantemp_JP$year<-as.numeric(annualmeantemp_JP$year)
#ggplot(annualmeantemp_JP, aes(x=year, y=annualmeantemp, color=pop)) + geom_point() + geom_line()

plot_grid(ggplot(annualmeantemp_US, aes(x=year, y=annualmeantemp, color=pop)) + geom_point() + geom_line() + stat_smooth(method=lm, se=FALSE), ggplot(annualmeantemp_JP, aes(x=year, y=annualmeantemp, color=pop)) + geom_point() + geom_line() + stat_smooth(method=lm, se=FALSE))
```


```{r figs_for_poster, include=FALSE, echo=FALSE}
ggplot(cpp_all, aes(x=lat, y=Estimate, color=country, shape=year)) + 
...
```{r}
ggplot(filter(cpp_2, year!=1988), aes(x=lat, y=Estimate, fill=country, shape=year)) + 
  geom_point(size=5) + scale_fill_manual(values=c("black", "gray"), name="Country") + scale_shape_manual(values=c(21,24), name="Year") +
  stat_smooth(method=lm, se=FALSE, aes(linetype=year, color=country)) + scale_linetype_manual(values=c("dotted","solid"), name="Year") + 
  scale_color_manual(values=c("black","gray"), name="Country") +
  guides(fill=guide_legend(override.aes=list(shape=22)), linetype=guide_legend(override.aes=list(color="black")),
  color=guide_legend(override.aes=list(linetype=0)), color=guide_legend(override.aes=list(linetype=0)), linetype=guide_legend(legend.key.width=unit(3,"line"))) +
  theme(legend.position=c(.8,.25), text=element_text(size=16),
        panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
        panel.background = element_blank(), axis.line = element_line(colour = "black")) + 
  xlab("Latitude") + ylab("Critical photoperiod (h)")

ggplot(filter(cpp_2, year!=1988), aes(x=lat, y=Estimate, fill=country, shape=year)) + 
  geom_point(size=5) + scale_fill_manual(values=c("black", "gray"), name="Country") + scale_shape_manual(values=c(21,22), name="Year") +
  stat_smooth(method=lm, se=FALSE, aes(linetype=year, color=country)) + scale_linetype_manual(values=c("dotted","solid"), name="Year") + 
  scale_color_manual(values=c("black","gray"), name="Country") +
  guides(fill=guide_legend(override.aes=list(shape=23)), linetype=guide_legend(override.aes=list(color="black"), legend.key.width=unit(3,"line")),
...
ggplot(filter(cpp_2, year!=1988), aes(x=lat, y=Estimate, fill=year, shape=country)) + 
  geom_point(size=5) + scale_fill_manual(values=c("gray", "black"), name="Year") + scale_shape_manual(values=c(21,22), name="Country") +
  stat_smooth(method=lm, se=FALSE, aes(linetype=country, color=year)) + scale_linetype_manual(values=c("dotted","solid"), name="Country") + 
  scale_color_manual(values=c("gray","black"), name="year") +
  guides(fill=guide_legend(override.aes=list(shape=22)), linetype=guide_legend(override.aes=list(color="black")),
  color=guide_legend(override.aes=list(linetype=0)), color=guide_legend(override.aes=list(linetype=0)), linetype=guide_legend(legend.key.width=unit(3,"line"))) +
  theme(legend.position=c(.8,.25), text=element_text(size=16),
        panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
...
ggplot(cpp_all, aes(x=lat, y=Estimate, alpha=year, fill=country)) +
  geom_point(size=3, shape=21, stroke=1) + scale_fill_manual(values=c("white", "black"), name="Country") +
  scale_alpha_manual(values=c(0.3, 1), name="Year") +
  stat_smooth(method=lm, se=FALSE, aes(linetype=country, color=year), size=1.5) + scale_color_manual(values=c("gray", "black"), name="Year") +  scale_linetype_manual(values=c("longdash","solid"), name="Country") + 
  theme(legend.position=c(.8,.25), text=element_text(size=16),
        panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
        panel.background = element_blank(), axis.line = element_line(colour = "black")) + 
  xlab("Latitude") + ylab("Critical photoperiod (h)") + 
...
ggplot(cpp_all, aes(x=lat, y=Estimate, alpha=year, color=country)) +
  geom_point(size=3) + scale_color_manual(values=c("blue", "black"), name="Country") +
  scale_alpha_manual(values=c(0.3, 1), name="Year") +
  stat_smooth(method=lm, se=FALSE, aes(linetype=year, color=country), size=2) + scale_linetype_manual(values=c("longdash","solid"), name="Country") + 
  theme(legend.position=c(.8,.25), text=element_text(size=16),
        panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
        panel.background = element_blank(), axis.line = element_line(colour = "black")) + 
  xlab("Latitude") + ylab("Critical photoperiod (h)") + 
